...
def run(total_epoch, best_loss):
    ...
    for step in range(total_epoch):
        ...
        if valid_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'transformer/saved/model-{0}-{1}.pt'.format(valid_loss,method))

        f = open(f'transformer/result/train_loss_{method}.txt', 'w')
        f.write(str(train_losses))
        f.close()

        f = open(f'transformer/result/bleu_{method}.txt', 'w')
        f.write(str(bleus))
        f.close()

        f = open(f'transformer/result/test_loss_{method}.txt', 'w')
